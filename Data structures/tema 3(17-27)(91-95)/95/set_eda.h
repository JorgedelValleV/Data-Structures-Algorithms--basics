...
public:
   ...
   ////////////////////////////anadido
   std::pair<bool, T>  lowerBound(T const & e) const {

	   return menorCota(raiz, e, false, false);

   }
   
protected:
	///////////////////////////////anadido
	std::pair<bool, T>  menorCota(Link r, T const & e, bool dcha, bool izqd) const {
		if (r == nullptr) {
			return { !dcha || (izqd&&dcha),  0 };
		}
		else if (!menor(e, r->elem) && !menor(r->elem, e)) {
			return { true,  r->elem };
		}
		else if (menor(e, r->elem)) {
			std::pair<bool, T> x = menorCota(r->iz, e,false,true);
			if(x.second == 0) return { x.first,  r->elem };//  && (r->iz!=nullptr && menor(r->iz->elem,e))
			else return { x.first,   x.second};
			//return { x.first,  r->elem };
		}
		else {//if (menor(r->elem, e)) 
			std::pair<bool, T> x = menorCota(r->dr, e,true,izqd);
			return { x.first , x.second };//&& (x.second != e || e==r->elem)
		}
	}
