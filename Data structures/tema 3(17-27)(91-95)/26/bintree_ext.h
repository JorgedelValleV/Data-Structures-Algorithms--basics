...
public:
	...
	bool arbolDeBusqueda() const {

		return arbolDeBusqueda(this->raiz, -1, -1);
	}
...
private:
	...
	static bool arbolDeBusqueda(Link r,int min, int max) {

		if (r == nullptr ) {
			return true;
		}
		else if ((min != -1 && r->elem <= min) || (max != -1 && r->elem >= max)) {
			return false;
		}
		else {
			return arbolDeBusqueda(r->left,min, r->elem) && arbolDeBusqueda(r->right, r->elem, max);
		}
	}
