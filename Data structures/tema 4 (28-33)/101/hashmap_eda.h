...
public:
	...
	iterator erase(iterator const& it) {
		assert(this == it.tabla); // comprueba que el iterador es de este mapa
		if (it == this->end())
			throw std::out_of_range("fuera del mapa");
		//Link aux = it.act;
		Link ant, pos = array[it.ind];
		localizar(it.act->cv.clave, ant, pos);
		iterator ret(this, pos->cv.clave);;
		++ret;
		if (ant == nullptr)
			array[it.ind] = pos->sig;
		else
			ant->sig = pos->sig;
		delete pos;
		--nelems;
		//this->erase(it.act->cv.clave);
		return ret;
	}
};
